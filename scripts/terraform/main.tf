...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "8081",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },     
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "v1.13.0" ? local.instance_image_id_maps_v1130[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v1170[data.huaweicloud_availability_zones.az.region] 
